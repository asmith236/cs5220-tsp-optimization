...
__global__
void tsp_kernel(double* dp, int* parent, double* distances, int n, int subset_size) {
    int mask = blockIdx.x;  // Each block handles one subset
    int u = threadIdx.x;    // Each thread handles one node in the subset

    if (mask >= (1 << n) || __popc(mask) != subset_size || u >= n) return;

    if (!(mask & (1 << u))) return; // Skip if `u` is not in subset `mask`
    ...
    for (int v = 0; v < n; v++) {
        if (!(prev_mask & (1 << v))) continue;  // Node `v` must be in the previous subset
        double new_cost = dp[prev_mask * n + v] + distances[v * n + u];

        // Debugging output for each thread and node
        // printf("Thread %d, Subset Mask: %d, Node: %d, Distance from %d to %d: %f\n", 
        //        threadIdx.x, mask, u, v, u, distances[v * n + u]);

        if (new_cost < min_cost) {
            min_cost = new_cost;
            best_parent = v;
            // Debugging the update
            // printf("Updating dp[%d][%d]: %f (Parent: %d)\n", mask, u, min_cost, best_parent);
        }
    }

    // Check for invalid memory access
    if (mask < (1 << n) && u < n) {
        if (dp[mask * n + u] != min_cost) {
            // printf("Updating dp[%d][%d] from %f to %f\n", mask, u, dp[mask * n + u], min_cost);
        }
